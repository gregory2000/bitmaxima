...
public class JavaUtilLoggingAdaptor {
  ...
  public static void main(String[] args) {
    if (args.length > 0 && args[0].equals("redwood")) {
      ...
      logger.log(Level.INFO, "RESTserver from the class logger");

      Redwood.log("RESTserver from Redwood!");
      Redwood.rootHandler().addChild(
        RedirectOutputHandler.fromJavaUtilLogging(topLogger));
      Redwood.log("RESTserver from Redwood -> Java!");
      Redwood.log("RESTserver from Redwood -> Java again!");
      logger.log(Level.INFO, "RESTserver again from the class logger");
      Redwood.startTrack("a track");
      Redwood.log("Inside a track");
      logger.log(Level.INFO, "RESTserver a third time from the class logger");
      Redwood.endTrack("a track");
      logger.log(Level.INFO, "RESTserver a fourth time from the class logger");
    }
  }
